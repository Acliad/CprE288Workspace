...
////Prepares a string for the lcd_rotatingBanner function
char *shift_left(char *str, char right_char);
...
void lcd_rotatingBanner(char dispText[]){
    ...
    //initially empty string with a spot for each character on the LCD
    char str[LCD_WIDTH + 1] = "                    ";

    int msg_len = strlen(dispText); // Gets the length of the passed in string
    int num_loops = msg_len + LCD_WIDTH; // Define the number of times to update the LCD

    int i;
    for (i = 0; i < num_loops; i++) {
        char new_char; // Character to be passed to shift left (LCD's right-most character)
        if (i < msg_len) { // If the loop hasn't reached the end of dispText, then set the new_char to be a char from dispText
            new_char = dispText[i];
        }
        else { // Otherwise, add a blank character (for shifting the message left)
            new_char = ' ';
        }

        shift_left(str, new_char); // Shift all elements of the display string left, appending new_char to the right
        lcd_printf("%s", str);
...
/**
 * Takes a char array and shifts each element left by 1, destroying the left-most element (str[0]). Appends the given char to the end of the array (str[length - 1]).
 */
char *shift_left(char *str, char right_char) {
    int len = strlen(str);
    int i;

    for (i = 0; i < len; i++) { // Shift all elements left by 1
        str[i] = str[i + 1];
    }
    str[len - 1] = right_char; // Append the given right_char to the end of the string
