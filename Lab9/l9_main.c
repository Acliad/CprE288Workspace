 ...
 */
void proccess_data(distance_t data[], int length) {
  object_t object[MAX_NUM_OBJECTS] = {
      0};                          // Initialize an array of empty objects
  ...
  int i = 0;
  for (i = 0; i < length; i++) {
    if (data[i].ir_distance <
        75.00) { // Look for spots where IR detects an object
      if (!processing) {
    ...
    } else if (processing) { // We found the falling edge of an object
      processing = 0;
      object[object_num].end_i = i;
      unsigned int num_samples = i - (object[object_num].start_i);
      if (num_samples > 0) { // upping this number will provide filtering
        ...
        float linear_width =
            distance * ang_width * M_PI / 180.00 * .3 * (0.032 * distance);
